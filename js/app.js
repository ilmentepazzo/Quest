...
function normalizeStoryInquiryMessage(row) {
  if (!row) return null;
  return {
    id: storyId(row.id),
    inquiryId: storyId(row.inquiry_id),
    senderId: storyId(row.sender_id),
    body: row.body || "",
    createdAt: row.created_at || "",
    source: "supabase"
...
async function loadSupabaseStoryInquiryMessages() {
  if (typeof supabaseClient === "undefined") return [];

  const userId = getCurrentUserId();
  if (!userId) {
    supabaseStoryInquiryMessagesCache = [];
    supabaseStoryInquiryMessagesLoaded = false;
    return [];
  }
  ...
  if (error) {
    const message = String(error.message || "");
    if (!message.toLowerCase().includes("does not exist") && !message.toLowerCase().includes("schema cache")) {
      console.warn("Impossibile caricare messaggi contatto Master:", error.message);
    }
    supabaseStoryInquiryMessagesCache = [];
    supabaseStoryInquiryMessagesLoaded = false;
    return [];
  ...
  supabaseStoryInquiryMessagesCache = (data || []).map(normalizeStoryInquiryMessage).filter(Boolean);
...
function formatInquiryCooldownRemaining(ms) {
  const totalMinutes = Math.max(1, Math.ceil(Number(ms || 0) / 60000));
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;

  if (hours > 0 && minutes > 0) {
    return tf("cooldownHoursMinutes", { hours, minutes }, `${hours} h ${minutes} min`);
  }
  if (hours > 0) {
    return hours === 1
      ? t("cooldownOneHour", "1 ora")
      : tf("cooldownHours", { hours }, `${hours} ore`);
  }
  return minutes === 1
    ? t("cooldownOneMinute", "1 minuto")
    : tf("cooldownMinutes", { minutes }, `${minutes} minuti`);
}

function hasCurrentUserStoryAccessForMessaging(storyIdValue, userId = getCurrentUserId()) {
  const normalizedStoryId = storyId(storyIdValue);
  if (!normalizedStoryId || !userId) return false;

  if (isStoryPurchasedByCurrentUser(normalizedStoryId) || isStoryUnlocked(normalizedStoryId)) return true;

  const privateAccess = getBookings().some(booking =>
    storyIdsMatch(booking.user_id, userId) &&
    storyIdsMatch(booking.storyId || booking.story_id, normalizedStoryId) &&
    !isBookingRejectedOrCancelled(booking.status) &&
    (isBookingAccepted(booking.status) || isBookingCompleted(booking.status)) &&
    (!isBookingExpired(booking) || isBookingCompleted(booking.status))
  );

  if (privateAccess) return true;

  return getJoinedPublicSessionProfileItems(userId).some(item =>
    storyIdsMatch(item.storyId, normalizedStoryId) &&
    (isBookingCompleted(item.status) || !item.expired)
  );
}

function getStoryConversationCooldownInfo(conversation, userId = getCurrentUserId()) {
  if (!conversation || !userId) return { allowed: true };
  if (!storyIdsMatch(userId, conversation.playerId)) return { allowed: true };
  if (hasCurrentUserStoryAccessForMessaging(conversation.storyId, userId)) return { allowed: true };

  const playerMessages = getConversationMessages(conversation.id)
    .filter(message => storyIdsMatch(message.senderId, userId) && message.createdAt)
    .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));

  const latest = playerMessages[0];
  if (!latest?.createdAt) return { allowed: true };

  const elapsed = Date.now() - new Date(latest.createdAt).getTime();
  const remainingMs = STORY_INQUIRY_COOLDOWN_MS - elapsed;
  if (remainingMs <= 0) return { allowed: true };

  return {
    allowed: false,
    remainingMs,
    label: formatInquiryCooldownRemaining(remainingMs),
    latest
  };
}

async function ensureConversationMessagesForCooldown() {
  if (supabaseConversationMessagesLoaded || typeof supabaseClient === "undefined") return;
  await loadSupabaseConversationMessages();
}

function showStoryInquiryCooldownToast(cooldownInfo) {
  const time = cooldownInfo?.label || t("cooldownFewHours", "qualche ora");
  showToast(
    tf("storyInquiryCooldownToast", { time }, `Hai già inviato una domanda recente per questa storia. Potrai aggiungere un nuovo messaggio tra ${time}.`),
    "warning"
  );
}
  ...
  return `
    <div class="story-card ${purchasedByCurrentUser ? "is-purchased" : ""} ${ownedByCurrentUser ? "is-owned" : ""} ${favoriteByCurrentUser ? "is-favorite" : ""}" onclick='openStory(${storyArg})'>
      ...
      <div class="story-card-body">
        ...
        <p class="story-card-description">${escapeHtml(story.desc || "")}</p>
        <button type="button" class="story-card-more-button" onclick='event.stopPropagation(); openStory(${storyArg})'>
          ${t("storyCardMoreDetails", "Mostra altro")}
        </button>
...
async function sendStoryInquiry(event) {
  ...
  try {
    ...
    await ensureConversationMessagesForCooldown();
    const cooldownInfo = getStoryConversationCooldownInfo(conversation, userId);
    if (!cooldownInfo.allowed) {
      showStoryInquiryCooldownToast(cooldownInfo);
      return;
    }
...
async function replyStoryInquiry(inquiryIdValue) {
  openStoryInquiryThreadModal(inquiryIdValue);
}

function getStoryInquiryById(inquiryIdValue) {
  const inquiryId = storyId(inquiryIdValue);
  return supabaseStoryInquiriesCache.find(item => storyIdsMatch(item.id, inquiryId)) || null;
}

function getStoryInquiryThreadMessages(inquiry) {
  if (!inquiry) return [];
  const inquiryId = storyId(inquiry.id);
  const storedMessages = supabaseStoryInquiryMessagesCache
    .filter(message => storyIdsMatch(message.inquiryId, inquiryId));

  const fallbackMessages = [];
  if (inquiry.message) {
    fallbackMessages.push({
      id: `${inquiryId}-initial`,
      inquiryId,
  ...
  if (inquiry.masterReply) {
    fallbackMessages.push({
      id: `${inquiryId}-reply`,
      inquiryId,
      senderId: inquiry.recipientId,
      body: inquiry.masterReply,
      createdAt: inquiry.repliedAt || inquiry.updatedAt,
      synthetic: true
    });
  }

  const merged = [...fallbackMessages, ...storedMessages];
  const seen = new Set();
  return merged
    .filter(message => {
      const key = `${storyId(message.senderId)}::${String(message.body || "").trim()}`;
...
function ensureStoryInquiryThreadModal() {
  let modal = document.getElementById("storyInquiryThreadModal");
  if (modal) return modal;

  modal = document.createElement("div");
  modal.id = "storyInquiryThreadModal";
  modal.className = "booking-messages-modal story-inquiry-thread-modal";
  modal.hidden = true;
  modal.innerHTML = `
    <div class="booking-messages-box story-inquiry-thread-box" role="dialog" aria-modal="true" aria-labelledby="storyInquiryThreadTitle">
      <div class="booking-messages-header">
        <div>
          <h2 id="storyInquiryThreadTitle">${escapeHtml(t("storyInquiryThreadTitle", "Conversazione"))}</h2>
          <p id="storyInquiryThreadSubtitle"></p>
        </div>
        <button class="light icon-button" type="button" onclick="closeStoryInquiryThreadModal()" aria-label="${escapeHtmlAttribute(t("commonCloseModal", "Chiudi"))}">×</button>
      </div>
      <div id="storyInquiryThreadMessages" class="story-inquiry-thread-messages"></div>
  ...
  modal.addEventListener("click", event => {
    if (event.target === modal) closeStoryInquiryThreadModal();
  });
  return modal;
}

function renderStoryInquiryThreadModal() {
  const inquiry = getStoryInquiryById(currentStoryInquiryThreadId);
  const modal = ensureStoryInquiryThreadModal();
  const title = document.getElementById("storyInquiryThreadTitle");
  const subtitle = document.getElementById("storyInquiryThreadSubtitle");
  const list = document.getElementById("storyInquiryThreadMessages");
  const input = document.getElementById("storyInquiryThreadInput");

  if (!inquiry || !list) return;

  const story = getAllStories().find(item => storyIdsMatch(item.id, inquiry.storyId));
  const currentUserId = getCurrentUserId();
  const otherUserId = storyIdsMatch(currentUserId, inquiry.senderId) ? inquiry.recipientId : inquiry.senderId;
  const otherName = getUserDisplayName(otherUserId, t("bookingParticipant", "partecipante"));

  if (title) title.textContent = story?.title || inquiry.story || t("storyInquiryThreadTitle", "Conversazione");
  if (subtitle) subtitle.textContent = tf("storyInquiryThreadSubtitle", { name: otherName }, `Con ${otherName}`);

  const messages = getStoryInquiryThreadMessages(inquiry);
  list.innerHTML = messages.map(message => {
    const mine = storyIdsMatch(message.senderId, currentUserId);
    const senderName = mine ? t("profileInquiryYourMessage", "Tu") : getUserDisplayName(message.senderId, otherName);
    return `
      <div class="story-inquiry-thread-message ${mine ? "is-mine" : "is-theirs"}">
        <div class="story-inquiry-thread-bubble">
          <strong>${escapeHtml(senderName)}</strong>
          <p>${escapeHtml(message.body)}</p>
          ${message.createdAt ? `<small>${escapeHtml(formatLocalizedDateTime(message.createdAt))}</small>` : ""}
        </div>
      </div>
    `;
  }).join("");
  list.scrollTop = list.scrollHeight;
  if (input && modal.hidden === false) input.focus();
}

async function openStoryInquiryThreadModal(inquiryIdValue) {
  const inquiryId = storyId(inquiryIdValue);
  if (!inquiryId) return;
  currentStoryInquiryThreadId = inquiryId;
  if (!supabaseStoryInquiryMessagesLoaded) await loadSupabaseStoryInquiryMessages();
  const modal = ensureStoryInquiryThreadModal();
  modal.hidden = false;
  modal.classList.add("open");
...
function closeStoryInquiryThreadModal() {
  const modal = document.getElementById("storyInquiryThreadModal");
  if (modal) {
    modal.hidden = true;
    modal.classList.remove("open");
  }
  currentStoryInquiryThreadId = null;
}

async function sendStoryInquiryThreadMessage(event) {
  event.preventDefault();
  const inquiry = getStoryInquiryById(currentStoryInquiryThreadId);
  const input = document.getElementById("storyInquiryThreadInput");
  ...
  const isMasterReply = storyIdsMatch(userId, inquiry.recipientId);
  const otherUserId = isMasterReply ? inquiry.senderId : inquiry.recipientId;
  const now = new Date().toISOString();

  const { data: messageRow, error: messageError } = await supabaseClient
    .from("story_inquiry_messages")
    .insert({ inquiry_id: inquiry.id, sender_id: userId, body })
    .select()
    .single();

  if (messageError) {
    showToast(`${t("storyInquiryReplyError", "Errore invio risposta")}: ${messageError.message}`, "error");
    return;
  }

  const patch = isMasterReply
    ? { status: "replied", master_reply: body, replied_at: now }
  ...
  if (updateError) {
    console.warn("Messaggio inviato, ma stato contatto non aggiornato:", updateError.message);
  }
  ...
  const updatedInquiry = normalizeStoryInquiry(updatedRow) || { ...inquiry, ...patch, updatedAt: now };
...
async function registerFreeStoryUnlockPurchase(story) {
  if (typeof supabaseClient === "undefined" || !story?.id || isStoryPaymentRequired(story)) return null;

  const userId = getCurrentUserId();
  if (!userId || isCurrentUserStory(story)) return null;

  const existing = getCurrentUserStoryPurchase(story.id);
  if (existing) return existing;

  const payload = {
    story_id: storyId(story.id),
    user_id: userId,
    master_id: getStoryAuthorId(story) || null,
    payment_status: "not_required",
    amount: 0,
    currency: "EUR",
    payment_amount: 0,
    payment_currency: "EUR",
    payment_provider: "free_unlock",
    payment_reference: `free-unlock-${userId}-${storyId(story.id)}`
  };
  ...
  if (error) {
    ...
    if (!message.includes("duplicate") && !message.includes("unique")) {
      console.warn("Sblocco gratuito salvato solo localmente:", error.message);
      return null;
    }
    ...
    if (selectError) {
      console.warn("Sblocco già presente, ma non leggibile:", selectError.message);
      return null;
    }

    const normalizedExisting = normalizeStoryPurchase(existingRows?.[0]);
    if (normalizedExisting) {
      supabaseStoryPurchasesCache = [normalizedExisting, ...supabaseStoryPurchasesCache.filter(item => !storyIdsMatch(item.id, normalizedExisting.id))];
      supabaseStoryPurchasesLoaded = true;
    }
    return normalizedExisting;
  }

  const normalized = normalizeStoryPurchase(data);
  if (normalized) {
    supabaseStoryPurchasesCache = [normalized, ...supabaseStoryPurchasesCache.filter(item => !storyIdsMatch(item.id, normalized.id))];
    supabaseStoryPurchasesLoaded = true;
  }
  return normalized;
}
...
async function unlockCurrentStory() {
  if (!currentStory) return;
  ...
  await registerFreeStoryUnlockPurchase(currentStory);
          ...
          return `
            <div class="card open-session-card ${isJoined ? "open-session-card-joined" : ""}" data-open-session-card="${escapeHtmlAttribute(session.id)}">
              ...
              <div class="open-session-content">
                ...
                <p class="open-session-description">${escapeHtml(story.desc || "")}</p>
                ...
                <div class="open-session-actions">
                  <button class="light" onclick='openStory(${storyArg})'>${t("storyCardMoreDetails", "Mostra altro")}</button>
                  ${isOwner
...
function renderDashboardStats() {
  ...
  const ratingText = document.getElementById("dashboardRatingText");
  if (!storiesCount && !ratingText) return;
  ...
  if (storiesCount) {
    storiesCount.textContent = totalStories === 1
      ? t("masterStoriesCountOne", "1 storia attiva")
      : tf("masterStoriesCountMany", { count: totalStories }, `${totalStories} storie attive`);
  }

  if (ratingText) {
    const reviews = getProfileReviews();
    if (!reviews.length) {
      ratingText.textContent = t("masterRatingTextEmpty", "Nessuna recensione ancora");
    } else {
      const average = reviews.reduce((sum, review) => sum + Number(review.rating || 0), 0) / reviews.length;
      ratingText.textContent = tf(
        "masterRatingTextDynamic",
        { rating: average.toFixed(1), count: reviews.length },
        `${average.toFixed(1)} ★ · ${reviews.length} ${reviews.length === 1 ? "recensione" : "recensioni"}`
      );
    }
  }
}
...
function getReviewForStoryByCurrentUser(storyIdValue, userIdValue = getCurrentUserId()) {
  const normalizedStoryId = storyId(storyIdValue);
  const normalizedUserId = storyId(userIdValue);
  if (!normalizedStoryId || !normalizedUserId) return null;

  return getStoryReviewsStore().find(review =>
    storyIdsMatch(review.storyId, normalizedStoryId) &&
    storyIdsMatch(review.user_id, normalizedUserId)
  ) || null;
}

function isStoryUnlockedForReview(story) {
  const id = storyId(story?.id || "");
  if (!id || isStoryPaymentRequired(story)) return false;
  return getUnlockedStories().some(unlockedId => storyIdsMatch(unlockedId, id));
}

function getCompletedReviewableBookingForStory(storyIdValue, userIdValue = getCurrentUserId()) {
  const normalizedStoryId = storyId(storyIdValue);
  const normalizedUserId = storyId(userIdValue);
  if (!normalizedStoryId || !normalizedUserId) return null;
  ...
  return getBookings().find(booking =>
    ...
    storyIdsMatch(booking.storyId, normalizedStoryId) &&
    storyIdsMatch(booking.user_id, normalizedUserId) &&
    isBookingCompleted(booking.status) &&
    hasCompletedBookingAccess(booking)
  ) || null;
}

function getCompletedReviewablePublicSessionForStory(storyIdValue, userIdValue = getCurrentUserId()) {
  const normalizedStoryId = storyId(storyIdValue);
  const normalizedUserId = storyId(userIdValue);
  if (!normalizedStoryId || !normalizedUserId) return null;

  return getJoinedPublicSessionProfileItems(normalizedUserId).find(item =>
    storyIdsMatch(item.storyId, normalizedStoryId) &&
    (isBookingCompleted(item.status) || (item.expired && hasCompletedBookingAccess(item)))
  ) || null;
}

function getPaidPurchaseForReview(storyIdValue, userIdValue = getCurrentUserId()) {
  const normalizedStoryId = storyId(storyIdValue);
  const normalizedUserId = storyId(userIdValue);
  if (!normalizedStoryId || !normalizedUserId) return null;

  return supabaseStoryPurchasesCache.find(purchase => {
    const status = normalizePaymentStatus(purchase.paymentStatus);
    return storyIdsMatch(purchase.user_id, normalizedUserId) &&
      storyIdsMatch(purchase.storyId, normalizedStoryId) &&
      ["paid", "not_required"].includes(status);
  }) || null;
}

function buildStoryReviewTarget(story, source, item = null) {
  if (!story) return null;
  const masterId = storyId(getStoryAuthorId(story) || story.author_id || story.owner_id || story.masterId || item?.masterId || "");
  return {
    source,
    backendSource: item?.source === "local" || story?.source === "local" ? "local" : "supabase",
    id: storyId(item?.id || story.id),
    bookingId: source === "booking" ? storyId(item?.id || "") : "",
    storyId: storyId(story.id || item?.storyId || ""),
    storyTitle: story.title || item?.story || t("storyFallback", "una storia"),
    masterId,
    dateLabel: item ? formatBookingDateTime(item.date, item.startTime || item.time, item.endTime) : ""
  };
}

function getReviewTargetFromBooking(booking) {
  if (!booking) return null;
  const story = getAllStories().find(item => storyIdsMatch(item.id, booking.storyId));
  const storyRecord = story || {
    id: booking.storyId,
    title: booking.story || t("storyFallback", "una storia"),
    author_id: booking.masterId
  };

  if (booking.source === "public_session") {
    if (!(isBookingCompleted(booking.status) || (booking.expired && hasCompletedBookingAccess(booking)))) return null;
    return buildStoryReviewTarget(storyRecord, "public_session", booking);
  }

  if (!isBookingCompleted(booking.status) || !hasCompletedBookingAccess(booking)) return null;
  return buildStoryReviewTarget(storyRecord, "booking", booking);
}

function getReviewEligibilityForStory(story = currentStory) {
  const userId = getCurrentUserId();
  const normalizedStoryId = storyId(story?.id || "");

  if (!story || !normalizedStoryId) {
    return { canReview: false, reason: "missing" };
  }
  ...
  if (storyIdsMatch(getStoryAuthorId(story) || story.author_id || story.owner_id || story.masterId, userId)) {
    return { canReview: false, reason: "owner" };
  }

  const existingReview = getReviewForStoryByCurrentUser(normalizedStoryId, userId);
  if (existingReview) {
    return { canReview: false, reason: "reviewed", review: existingReview };
  }

  const completedBooking = getCompletedReviewableBookingForStory(normalizedStoryId, userId);
  if (completedBooking) {
    return { canReview: true, reason: "booking", target: buildStoryReviewTarget(story, "booking", completedBooking) };
  }

  const completedSession = getCompletedReviewablePublicSessionForStory(normalizedStoryId, userId);
  if (completedSession) {
    return { canReview: true, reason: "public_session", target: buildStoryReviewTarget(story, "public_session", completedSession) };
  }

  const paidPurchase = getPaidPurchaseForReview(normalizedStoryId, userId);
  if (paidPurchase) {
    return { canReview: true, reason: "purchase", target: buildStoryReviewTarget(story, "purchase", paidPurchase) };
  }

  if (isStoryUnlockedForReview(story)) {
    return { canReview: true, reason: "free_unlock", target: buildStoryReviewTarget(story, "free_unlock") };
  }

  return { canReview: false, reason: "not_available" };
}

function canReviewBooking(booking) {
  const target = getReviewTargetFromBooking(booking);
  return Boolean(target && !getReviewForStoryByCurrentUser(target.storyId));
}
...
function renderBookingReviewAction(booking, className = "light compact-action") {
  ...
  const existingReview = getReviewForStoryByCurrentUser(target.storyId) || (target.bookingId ? getReviewForBooking(target.bookingId) : null);
  if (existingReview) {
    return `<span class="profile-info-chip rating-chip">${t("reviewAlreadySent", "Recensita")} · ${"★".repeat(Number(existingReview.rating || 0))}</span>`;
  }

  if (booking?.source === "public_session") {
    return `<button class="${escapeHtmlAttribute(className)}" type="button" onclick='openStoryReviewModal(${JSON.stringify(storyId(target.storyId))})'>${escapeHtml(t("reviewLeaveButton", "Lascia recensione"))}</button>`;
  }

  if (!canReviewBooking(booking)) return "";
  return `<button class="${escapeHtmlAttribute(className)}" type="button" onclick='openReviewModal(${JSON.stringify(storyId(booking.id))})'>${escapeHtml(t("reviewLeaveButton", "Lascia recensione"))}</button>`;
}

function getReviewEligibilityMessage(eligibility) {
  if (!eligibility) return "";
  if (eligibility.reason === "login") return t("reviewEligibilityLogin", "Accedi per lasciare una recensione verificata.");
  if (eligibility.reason === "owner") return t("reviewEligibilityOwner", "Non puoi recensire una storia creata da te.");
  if (eligibility.reason === "reviewed") return t("reviewEligibilityReviewed", "Hai già recensito questa storia.");
  return t("reviewEligibilityLocked", "Potrai lasciare una recensione dopo aver acquistato, sbloccato o giocato questa storia.");
}

function renderStoryReviewAction(story = currentStory) {
  const container = document.getElementById("storyReviewAction");
  if (!container) return;

  const eligibility = getReviewEligibilityForStory(story);
  if (eligibility.canReview) {
    container.innerHTML = `<button class="light compact-action" type="button" onclick='openStoryReviewModal(${JSON.stringify(storyId(story.id))})'>${escapeHtml(t("reviewLeaveButton", "Lascia recensione"))}</button>`;
    return;
  }

  if (eligibility.reason === "reviewed") {
    const rating = Number(eligibility.review?.rating || 0);
    container.innerHTML = `<span class="profile-info-chip rating-chip">${escapeHtml(t("reviewAlreadySent", "Recensita"))}${rating ? ` · ${"★".repeat(rating)}` : ""}</span>`;
    return;
  }

  if (["owner", "not_available"].includes(eligibility.reason)) {
    container.innerHTML = `<small class="story-review-eligibility-note">${escapeHtml(getReviewEligibilityMessage(eligibility))}</small>`;
    return;
  }
...
function renderReviewModal(target) {
  if (!target?.storyId || !target?.masterId) {
    showToast(t("reviewNotAvailableToast", "Recensione non disponibile."), "warning");
    return;
  }

  closeReviewModal();
  currentReviewContext = target;
  ...
  modal.innerHTML = `
    <div class="review-modal-box" role="dialog" aria-modal="true" aria-labelledby="bookingReviewTitle">
      <div class="review-modal-header">
        <div>
          <p class="eyebrow">${escapeHtml(t("reviewModalEyebrow", "Recensione verificata"))}</p>
          <h2 id="bookingReviewTitle">${escapeHtml(t("reviewModalTitle", "Com’è stata l’esperienza?"))}</h2>
          <p>${escapeHtml(target.storyTitle || t("storyFallback", "una storia"))}${target.dateLabel ? ` · ${escapeHtml(target.dateLabel)}` : ""}</p>
        </div>
      ...
      <div class="review-modal-actions">
        ...
        <button class="primary" type="button" onclick="submitCurrentReview()">${escapeHtml(t("reviewSubmitButton", "Pubblica recensione"))}</button>
      </div>
    </div>
  `;
...
function openReviewModal(bookingIdValue) {
  ...
  if (!target || !canReviewBooking(booking)) {
    const existingReview = target ? getReviewForStoryByCurrentUser(target.storyId) || getReviewForBooking(bookingId) : getReviewForBooking(bookingId);
    showToast(existingReview ? t("reviewAlreadySentToast", "Hai già recensito questa storia.") : t("reviewNotAvailableToast", "Puoi recensire solo esperienze completate o sbloccate."), "warning");
    return;
  }
...
function openStoryReviewModal(storyIdValue) {
  const story = getAllStories().find(item => storyIdsMatch(item.id, storyIdValue)) || currentStory;
  const eligibility = getReviewEligibilityForStory(story);

  if (!eligibility.canReview) {
    showToast(getReviewEligibilityMessage(eligibility), "warning");
    return;
  }

  renderReviewModal(eligibility.target);
}
...
async function submitCurrentReview() {
  const target = currentReviewContext;
  if (!target?.storyId || !target?.masterId) {
    showToast(t("reviewNotAvailableToast", "Recensione non disponibile."), "warning");
    closeReviewModal();
    return;
  ...
  const alreadyReviewed = getReviewForStoryByCurrentUser(target.storyId);
  if (alreadyReviewed) {
    showToast(t("reviewAlreadySentToast", "Hai già recensito questa storia."), "warning");
    closeReviewModal();
    return;
  }
  ...
  const payload = {
    ...
    story_id: storyId(target.storyId),
    master_id: storyId(target.masterId),
    user_id: getCurrentUserId(),
    rating,
    comment
  };

  const shouldUseSupabase = typeof supabaseClient !== "undefined" && target.backendSource !== "local";
  ...
  if (shouldUseSupabase) {
    ...
    if (error) {
      const message = String(error.message || "");
      showToast(message.includes("duplicate") || message.includes("unique") ? t("reviewAlreadySentToast", "Hai già recensito questa storia.") : `${t("reviewSaveError", "Errore salvataggio recensione")}: ${error.message}`, "error");
      return;
    }
    ...
    localReviews.unshift({
      ...
      storyId: storyId(target.storyId),
      masterId: storyId(target.masterId),
      user_id: getCurrentUserId(),
